...
function vtkImageSlice(publicAPI, model) {
  ...
  publicAPI.requestData = (inData, outData) => {
    ...
    const xDim = imageDims[0];
    const xyDim = imageDims[0] * imageDims[1];
    for (let i = 0; i < size; ++i) {
      const ijk = [i % sliceDims[0], Math.floor(i / sliceDims[0])];
      ijk.splice(model.axis, 0, slice);
      const imageIndex = ijk[0] + ijk[1] * xDim + ijk[2] * xyDim;

      sliceBuffer[i] = buffer[imageIndex];
    }
